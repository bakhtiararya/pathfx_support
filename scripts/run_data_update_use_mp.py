...
# updated 1-8-19 to apply tested multiprocessing
# testing on toy model data
# 2-17-19 JLW
...
int_fname = '../prototyping/toy_graph.txt'
...
#########################################################################
# create binned data for multiprocessing
cmd = "python bin_data_mp.py -data_file %s -a_name %s -num_cores %s"%(unn,dup_name,num_cores)
print(cmd)
output = os.popen(cmd).read()
bin_out = get_data_files(output) 
genes_by_bin = bin_out['genes_by_bin']
bins_by_genes = bin_out['bins_by_genes']
...
thr_hashes = {}
thr_values = {}
# for thr in [0.99,0.95,0.9,0.89,0.88,0.87,0.86,0.85,0.84,0.83,0.82]:
for thr in [0.78]:
	### run DFS at each threshold
	cmd = "python run_depth_first_search_mp.py -intome %s -a_name %s -thr %s -gbbin %s"%(interactome,dup_name,thr,genes_by_bin)
	print(cmd)
	output = os.popen(cmd).read()
	...
	### old method ### cmd = "python calc_dfs_specificity.py -n2hash %s -a_name %s -thr %s -n2nbhd %s -thr_dir %s"%(n2hash,dup_name,str(thr),node_to_nbhd,thr_dir)
	### old method ### output = os.popen(cmd).read()
	### old method ### print('calculated specificity at: '+str(thr)+'\t'+'{0:%Y-%m-%d %H:%M:%S}'.format(datetime.datetime.now()))
	cmd = "python calc_dfs_specificity_h5py.py -n2hash %s -a_name %s -thr %s -n2nbhd %s -thr_dir %s -gbbin %s"%(n2hash,dup_name,str(thr),node_to_nbhd,thr_dir,genes_by_bin)
	print(cmd)
	output = os.popen(cmd).read()
...
print('\nBeginning phenotype calculations')
...
print('\nCalculating expected p-value distributions')
cmd = "python calc_exp_pvale_dist.py --spec_dic_hash_file %s --num_targ %s --cui_2_genes %s --genes_2_cui %s --intome_size %s --dup_name %s"%(sp_dic_hash,num_targ,cui_to_genes_file,gene_to_cui_file,intome_size,dup_name)
print(cmd)
output = os.popen(cmd).read()
...
cmd = 'python phenotype_enrichment_pathway_%s.py -d Metformin -a test_data_update'%(dup_name)
print(cmd)
