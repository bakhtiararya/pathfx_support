...
def find_all_files(dupnm,pdir,thr):
	...
	assimg = [f for f in os.listdir(rscs_dir) if dupnm in f and 'png' in f] # assessment images
	rsfs = [f for f in os.listdir(rscs_dir) if dupnm in f and 'pkl' in f] # get resources files
	rm_files = ['_r.pkl','thresh_hash_dic.pkl','thresh_val_dic.pkl','thr_dir.pkl','thr_values.pkl']
